...
def prepare_dataframe(df: pd.DataFrame, file_path: Path) -> pd.DataFrame:
    """
    Standardizes input columns for Planting, Harvesting, and Emergence dates.

    Args:
        df: Raw input DataFrame.
        file_path: Path to the source file (used for crop-type heuristics).
...
def process_and_save_temp(args: Tuple[int, pd.Series, NasaWeatherV3]) -> bool:
    """
    Processes a single dataset row and saves to temporary parquet.

    Args:
        args: A tuple containing (index, row_series, nasa_v3_instance).
    ...
    if df_w is not None:
        ...
        sheet_name = f"Row_{idx}_{date_label}"
